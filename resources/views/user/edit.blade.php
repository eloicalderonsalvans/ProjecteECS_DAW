        ...
        <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-8 flex flex-col items-center">
                <div class="relative group mb-4">
                    @if($user->getAvatarUrl())
                        <img src="{{ $user->getAvatarUrl() }}" alt="Foto de {{ $user->nom }}"
                            style="width: 112px; height: 112px; border-radius: 50%; object-fit: cover; border: 4px solid #dbeafe; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1); flex-shrink: 0;">
                    @else
                        <div style="width: 112px; height: 112px; border-radius: 50%; background: linear-gradient(135deg, #3b82f6, #4f46e5); display: flex; align-items: center; justify-content: center; color: white; font-weight: 800; font-size: 2.25rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1); border: 4px solid #dbeafe; flex-shrink: 0;">
                            {{ $user->getInicials() }}
                        </div>
                    @endif
                ...
                @if($user->getAvatarUrl())
                    <label
                        class="mt-2 flex items-center gap-2 text-xs text-red-500 cursor-pointer hover:text-red-700 transition-colors">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="cognom" value="{{ old('cognom', $user->cognom) }}" required
                        class="mt-1 block w-full border border-gray-300 rounded-md p-2">
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">Nova Contrassenya (deixar en blanc per
                        mantenir)</label>
                ...
                <div>
                    ...
                    <input type="date" name="data_alta" value="{{ old('data_alta', $user->data_alta->format('Y-m-d')) }}"
                        class="mt-1 block w-full border border-gray-300 rounded-md p-2">
                </div>
